    ...
    return <div className="w-[90%] sm:w-[30rem]">{
        keyboard.map((r, i) => {
            ...
            return <div key={i} className="flex ">{r.split("").map((letter, j) => {
                ...
                return <div key={j} onClick={() => props.onClick(text)} className={`flex leading-10 bg-gray-200 h-10 m-0.5 ${grow} rounded cursor-pointer font-bold text-center basis-0`}>
